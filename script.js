...
function init() {
    // 1. Inicializar Mapa primero para evitar errores de contenedor
...
async function cargarDatosMaestros() {
    ...
    try {
        ...
        // Crear diccionario de turbinas con horas
        diccionarioTurbinas = {};
        ...
        // 3. Procesar PLANIFICACION (Lógica de corte corregida)
        ...
        // Dibujar mapa solo con lo que está instalado hoy
...
function dibujarMapa(registros) {
    ...
    registros.forEach(r => {
        ...
        if (coords) {
            ...
            // Crear popup con diseño industrial y HORAS
            const popupContent = `
                ...
                <div class="map-label-body">
                    <div class="map-label-row">
                        <span class="map-label-tag">Horas Actuales:</span>
                        <span class="map-label-val">${r.horas} hrs</span>
                    </div>
                    <div class="map-label-row">
                        <span class="map-label-tag">Familia:</span>
                        <span class="map-label-val">${r.familia}</span>
                    </div>
                    <div class="map-label-row">
                        <span class="map-label-tag">Próximo OHL:</span>
                        <span class="map-label-val">${formatMMYYYY(r.proxOHL)}</span>
                    </div>
                </div>
            `;
...
function showView(viewId, familia = null) {
    ...
    if (viewId === 'home-view') {
        ...
        dibujarGantt(todosLosRegistros.filter(r => r.familia === String(familia).toUpperCase() || r.esRepa));
    }
}
            ...
            if (end > start) {
                ...
                bar.onmouseenter = (e) => {
                    ...
                    tooltip.innerHTML = `
                        ...
                        <div class="tooltip-row"><span class="tooltip-label">Estado:</span> <span class="tooltip-val">${m.esRepa ? 'REPA USA' : (m.esPlan ? 'Planificado' : 'En Operación')}</span></div>
